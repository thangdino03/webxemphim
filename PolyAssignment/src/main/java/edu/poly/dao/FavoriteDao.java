...
//		String jpql = "select new edu.poly.doman.FavoriteReport(f.video.title, count(f), min(f.likeDate), max(f.likeDate) from Favorite f group by f.video.title";
//		EntityManager em = JpaUtils.getEntityManager();
...
//	}
	public List<FavoriteReport> reportFavoritesByVideo() {
		String jpql = "select new edu.poly.domain.FavoriteReport(f.video.title, count(f),max(f.likedDate), min(f.likedDate)) from Favorite f group by f.video.title";
		EntityManager em = JpaUtils.getEntityManager();
		List<FavoriteReport> list = null;
		try {
			TypedQuery<FavoriteReport> query = em.createQuery(jpql, FavoriteReport.class);
